...
public class EMVideoView extends RelativeLayout implements AudioCapabilitiesReceiver.Listener {
    ...
    private RenderBuilder getRendererBuilder(VideoType renderType, Uri uri, MediaUtil.MediaType defaultMediaType) {
        switch (renderType) {
            ...
            default:
                return new RenderBuilder(getContext().getApplicationContext(), getUserAgent(), uri.toString(), defaultMediaType);
        }
    }
    ...
    public void setEmExoPlayer(EMExoPlayer emExoPlayer) {
        if (emExoPlayer == null) {
            if (this.emExoPlayer != null) {
                this.emExoPlayer.removeListener(listenerMux);
                this.emExoPlayer.setMetadataListener(null);
                this.emExoPlayer.blockingClearSurface();
                this.emExoPlayer = null;
            }
        } else {
            this.emExoPlayer = emExoPlayer;
        }
    }

    public EMExoPlayer getEmExoPlayer() {
        return emExoPlayer;
    }
     ...
     */
    private class EMExoVideoSurfaceCallback implements TextureView.SurfaceTextureListener {
